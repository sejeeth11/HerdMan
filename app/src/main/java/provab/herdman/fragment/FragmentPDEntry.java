 ...
 */
public class FragmentPDEntry extends Fragment {
    ...
    TextView ownerId;
    //TextView animalId;
    LinearLayout firsPdResultLayout;
    LinearLayout secondPdResultLayout;
    LinearLayout checkDateLayout;
    TextView firstPdResult;
    TextView secondPdResult;
    TextView checkDate;
    Button pdEntrySubmit;
    Button pdEntryCancel;
    ...
    private SlideDateTimeListener listener = new SlideDateTimeListener() {
        ...
        public void onDateTimeSet(Date date) {
            checkDate.setText(GlobalVar.dialogeFormat.format(date));
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_pd_entry, container, false);
        findViews(v);
        return v;
    }
    ...
    public void findViews(View view) {
     ...
     //   animalId = (TextView)view.findViewById(R.id.idNumber);
        ...
        firsPdResultLayout = (LinearLayout) view.findViewById(R.id.firsPdResultLayout);
        secondPdResultLayout = (LinearLayout) view.findViewById(R.id.secondPdResultLayout);
        checkDateLayout = (LinearLayout) view.findViewById(R.id.checkDateLayout);
        firstPdResult = (TextView) view.findViewById(R.id.firstPdResult);
        secondPdResult = (TextView) view.findViewById(R.id.secondPdResult);
        checkDate = (TextView) view.findViewById(R.id.checkDate);
        pdEntrySubmit = (Button) view.findViewById(R.id.pdEntrySubmit);
        pdEntryCancel = (Button) view.findViewById(R.id.pdEntryCancel);
        ...
        firsPdResultLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Dialog dialogPd1 = showPd1();
                dialogPd1.show();
            }
        });
        secondPdResultLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (firstPdResult.getTag() != null) {
                    Dialog dialogPd2 = showPd2();
                    dialogPd2.show();
                } else {
                    Toast.makeText(getActivity(), "PLEASE SELECT FIRST PD RESULT", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        checkDateLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try {
        ...
        pdEntrySubmit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Resources resources = getActivity().getResources();
                if (firstPdResult.getTag() == null || firstPdResult.getText().toString().equals(resources.getString(R.string.select_first_pd_result))) {
                    Toast.makeText(getActivity(), "PLEASE SELECT FIRST PD RESULT", Toast.LENGTH_LONG);
                    return;
                }
                if (secondPdResult.getTag() == null || secondPdResult.getText().toString().equals(resources.getString(R.string.select_second_pd_result))) {
                    Toast.makeText(getActivity(), "PLEASE SELECT FIRST PD RESULT", Toast.LENGTH_LONG);
                    return;
                }
                if (checkDate.getText().toString().equals(resources.getString(R.string.select_pd_date))) {
                    Toast.makeText(getActivity(), "PLEASE SELECT PD DATE", Toast.LENGTH_LONG);
                    return;
                }
                try {
                    long status = DatabaseHelper.getDatabaseHelperInstance(MyApplication.getContext()).savePDEntry(firstPdResult.getTag().toString(), secondPdResult.getTag().toString(), GlobalVar.databaseFormat.format(GlobalVar.dialogeFormat.parse(checkDate.getText().toString())), ownerId.getText().toString());
                    if (status >= 0) {
        ...
        pdEntryCancel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                getActivity().onBackPressed();
            }
...
//            lotList=DatabaseHelper.getDatabaseHelperInstance(getActivity()).getMilkAnimalStatus(animalId.getText().toString());
    ...
    private Dialog showPd1() {
        ...
        final ArrayList<String> lotList = DatabaseHelper.getDatabaseHelperInstance(getActivity()).getPDI();
        SpinnerAdapter adapter = new SpinnerAdapter(getActivity(), R.layout.spinner_content, lotList);
        title.setText("Select PDI");
        list.setAdapter(adapter);
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                firstPdResult.setText(((TextView) view).getText());
                firstPdResult.setTag(view.getTag().toString());
                dialog.dismiss();
            }
        });
        return dialog;
    ...
    private Dialog showPd2() {
        final Dialog dialog = new Dialog(getActivity());
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.spinner_dialog);
        final ListView list = (ListView) dialog.findViewById(R.id.dialogListView);
        final TextView title = (TextView) dialog.findViewById(R.id.tv_dialog_title);
        final ArrayList<String> lotList = DatabaseHelper.getDatabaseHelperInstance(getActivity()).getPDII();
        SpinnerAdapter adapter = new SpinnerAdapter(getActivity(), R.layout.spinner_content, lotList);
        title.setText("Select PDII");
        list.setAdapter(adapter);
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                secondPdResult.setText(((TextView) view).getText());
                secondPdResult.setTag(view.getTag().toString());
                dialog.dismiss();
            }
        });
        return dialog;
